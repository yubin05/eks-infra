...
# kubectl에서 EKS 클러스터를 조회/인증하기 위한 최소 권한
...
# SSH 포트를 열지 않고도 Session Manager로 Bastion에 접속할 수 있도록 부여
...
# eksctl로 클러스터를 만들면 자동으로 붙는 OIDC 연동/IRSA를
# Terraform으로 직접 만드는 부분. ALB Ingress가 동작하려면 필수.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# GitHub Actions 전용 IAM 사용자
# eks-app CI가 ECR에 이미지를 푸시할 수 있도록 최소 권한만 부여
# ---------------------------------------------------------------------------
